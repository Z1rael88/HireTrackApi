...
public class ResumeService( IResumeRepository resumeRepository, ICandidateRepository candidateRepository,IVacancyRepository vacancyRepository, IRepository<VacancyResume> vacancyResumeRepository,
    ...
    : IResumeService
{
    public async Task<ResumeResponseDto?> CreateResumeAsync(ResumeRequestDto dto)
    {
        ...
        var existingResume = await resumeRepository.GetResumeByCandidateEmail(resume.Candidate.Email);
        if (dto.VacancyId is not 0 && existingResume is not null)
        {
            ...
            await vacancyResumeRepository.CreateAsync(vacancyResumes);
            return null;
        }

        var createdResume = await resumeRepository.CreateAsync(resume);
        ...
        if (dto.VacancyId != 0)
        {
            ...
            await vacancyResumeRepository.CreateAsync(vacancyResume);
        }


        var user = await userManager.FindByEmailAsync(resume.Candidate.Email);
        var candidate = await candidateRepository.GetCandidateByEmailAsync(resume.Candidate.Email);
        if (user is not null && candidate is not null)
        {
            candidate.UserId = user.Id;
            await candidateRepository.UpdateAsync(resume.Candidate);
        }
    ...
    public async Task<ResumeResponseDto> GetResumeByIdAsync(int resumeId)
    {
        ...
        result.Status = await vacancyRepository.GetResumeStatusByResumeIdAsync(resumeId);
        return result;
    }

    public async Task<IEnumerable<ResumeResponseDto>> GetAllResumesByVacancyIdAsync(int vacancyId)
    {
        var resumes = await resumeRepository.GetAllResumesByVacancyId(vacancyId);
    ...
    {
        var vacancyResumes = await resumeRepository.GetVacancyResumeByIds(vacancyId, resumeId);
        vacancyResumes.Status = status;
        await vacancyResumeRepository.SaveChangesAsync();
    ...
    {
        validator.ValidateAndThrow(dto);
        await resumeRepository.UpdateResume(dto.Adapt<Resume>(), resumeId);
    }
    ...
    public async Task<ResumeResponseDto> GetResumeByUserIdAsync(int userId)
    {
        ...
        var result = resume.Adapt<ResumeResponseDto>();
        if (resume != null) result.Status = await vacancyRepository.GetResumeStatusByResumeIdAsync(resume.Id);
        return result;
    }
